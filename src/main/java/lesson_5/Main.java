...
public class Main {
    ...
    public static void main(String[] args) {
        System.out.println("ВАЖНОЕ ОБЪЯВЛЕНИЕ >>> Подготовка!!!");
        Race race = new Race(new Road(60), new Tunnel(CARS_COUNT), new Road(40, true));
        Car[] cars = new Car[CARS_COUNT];
        for (int i = 0; i < cars.length; i++) {
            cars[i] = new Car(race, 20 + (int) (Math.random() * 10));
        }

        ExecutorService executorService = Executors.newFixedThreadPool(cars.length);  //создаем объект для управления пулом потоков
        for (int i = 0; i < cars.length; i++) {
            executorService.execute(new Thread(cars[i])); //через метод execute запускаем поток на исполнение
        }
        executorService.shutdown();                       //упорядоченное завершение работы, дожидаемся корректного выполнения всех задач
        while (!executorService.isTerminated()){;}          // если все задачи исполнителя завершены по команде shutdown, выходим из цикла
        System.out.println("ВАЖНОЕ ОБЪЯВЛЕНИЕ >>> Гонка закончилась!!!");
    }
}
